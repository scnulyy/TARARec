...
for set in ['train', 'valid', 'test']:
    df = pd.read_csv(f"data/movie/ml-25m/{set}.csv")
    df = df.reset_index(drop=True)
    all_indice = []
    for idx, row in tqdm(df.iterrows()):
        tgt_id = int(row['movie_id'])
        hist_id = [int(i) for i in eval(row['history_movie_id'])]
        ...
        # 提取特征信息
        target_feature = all_movie_features[str(tgt_id)]
        hist_features = [all_movie_features[str(i)] for i in hist_id]
    ...
    json.dump(all_indice, open(f'./embeddings/ml-25m_{args.pooling}_indice_{set}.json', 'w'), indent=4)
